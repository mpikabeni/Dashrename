...
async def edit_callback_message(
    ...
    """
    Edite correctement un message texte OU la légende d'un message avec photo.
    Telegram refuse edit_message_text() sur un message qui ne contient pas de texte.
    """
    kwargs = {}
    ...
    try:
        ...
        return await edit_callback_message(query, 
            text=text,
            **kwargs,
        )
    except TelegramError as exc:
        # Dernier secours : si Telegram refuse l'édition du média,
        # envoyer un nouveau message au lieu de faire échouer l'action.
        print("DASH EDIT ERROR:", repr(exc))
        if query.message:
...
async def require_nexa(update: Update, context: ContextTypes.DEFAULT_TYPE) -> bool:
    ...
    if update.callback_query:
        await edit_callback_message(update.callback_query, text, parse_mode="HTML", reply_markup=subscription_keyboard())
    else:
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "thumb:extract":
        await edit_callback_message(query, "🖼️ Extraction de la vignette...")
...
def main():
    ...
    async def post_init(app: Application):
        # post_init est appelé avant que PTB marque l'application comme
        # "running". asyncio.create_task() évite le warning PTB ici.
