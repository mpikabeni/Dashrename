...
# 0 = aucune limite applicative. La limite effective vient du Local Bot API.
MAX_DOWNLOAD_BYTES = int(os.getenv("MAX_DOWNLOAD_BYTES", "0"))
MAX_PROCESS_SECONDS = int(os.getenv("MAX_PROCESS_SECONDS", "7200"))
...
TELEGRAM_LOCAL_API = os.getenv("TELEGRAM_LOCAL_API", "true").strip().lower() in {"1", "true", "yes", "on"}
TELEGRAM_API_BASE_URL = os.getenv("TELEGRAM_API_BASE_URL", "http://127.0.0.1:8081/bot").rstrip("/")
TELEGRAM_API_FILE_BASE_URL = os.getenv("TELEGRAM_API_FILE_BASE_URL", "http://127.0.0.1:8081/file/bot").rstrip("/")
...
rename_history: dict[int, list[dict]] = {}
...
async def menu_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.effective_message.reply_text(
        "⚡ <b>Dash FileBot</b>\n\n"
        "Envoie un fichier pour afficher les actions disponibles.\n\n"
        "/rename — renommer le dernier fichier\n"
        "/thumbnail — gérer la vignette\n"
        "/history — historique des renommages\n"
        "/settings — paramètres\n"
        "/status — état du bot\n"
        "/about — à propos\n"
        "/help — aide\n"
...
async def rename_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not session_for(user_id):
        await update.effective_message.reply_text("⚠️ Envoie d'abord un fichier.")
        return
...
async def thumbnail_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    session = session_for(update.effective_user.id)
    if not session:
        await update.effective_message.reply_text("⚠️ Envoie d'abord une vidéo.")
        return
    if session.get("kind") != "video":
        await update.effective_message.reply_text("⚠️ /thumbnail nécessite une vidéo.")
        return
    await update.effective_message.reply_text(
...
async def history_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    items = rename_history.get(update.effective_user.id, [])
    if not items:
        await update.effective_message.reply_text("📜 Aucun renommage enregistré.")
        return
    lines = ["📜 <b>Historique des renommages</b>", ""]
    for i, item in enumerate(reversed(items), 1):
        lines.append(f"{i}. <code>{item['old']}</code> → <code>{item['new']}</code>")
...
async def about_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.effective_message.reply_text(
        "⚡ <b>Dash FileBot</b>\n\n"
        "Bot de gestion, renommage, conversion, compression et traitement de fichiers.\n\n"
        "Créé par <b>NEXA</b>.\n"
        "Canal : @Nexa_CG",
        parse_mode="HTML",
    )
...
async def receive_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 0 = aucune limite applicative. Le Local Bot API autorise le téléchargement sans limite.
    if MAX_DOWNLOAD_BYTES > 0 and size > MAX_DOWNLOAD_BYTES:
        await update.message.reply_text(
            f"⚠️ Fichier trop volumineux.\n"
            f"Limite configurée : {format_size(MAX_DOWNLOAD_BYTES)}"
        )
        return
...
async def perform_rename(update: Update, context: ContextTypes.DEFAULT_TYPE,
    ...
    try:
        ...
        stats["renames"] += 1
        rename_history.setdefault(user_id, []).append({
            "old": session["name"],
            "new": safe_filename(new_name),
        })
        rename_history[user_id] = rename_history[user_id][-20:]
        try:
            await progress.edit_text(
                "✏️ <b>DASH RENOMMAGE</b>\n\n"
                "▰▰▰▰▰▰▰▰▰▰ <b>100%</b>\n"
...
async def admin_callbacks(update, context, data):
    ...
    if action == "settings":
        await query.edit_message_text(
            ...
            f"Limite de téléchargement : {'Aucune (Local Bot API)' if MAX_DOWNLOAD_BYTES <= 0 else format_size(MAX_DOWNLOAD_BYTES)}\n"
...
def main():
    ...
    if TELEGRAM_LOCAL_API:
        print(f"🔌 Local Bot API: {TELEGRAM_API_BASE_URL}")
        builder = (
            builder
            .base_url(TELEGRAM_API_BASE_URL)
            .base_file_url(TELEGRAM_API_FILE_BASE_URL)
            .local_mode(True)
        )
